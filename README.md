...
- **Windows**: PBO-accelerated `glBlitFramebuffer` + async `glReadPixels` into Media Foundation (works on all GPUs including Intel integrated). `supportsGpuInput()` always returns true. **TODO**: D3D11 interop via `WGL_NV_DX_interop2` (zero-copy GL→D3D11→MF) is implemented but disabled — `WriteSample` returns `E_INVALIDARG` on Intel Arc (driver falsely reports interop support) and needs testing on real NVIDIA hardware. See `media_foundation_encoder.cpp` for details.
